...
class network(nn.Module):
	def __init__(self):
		...
		self.w1 = nn.Parameter(torch.randn(2, 4) * 0.01) # 输入为2个特征,隐藏层神经元个数 4注意这里要携程nn.para 不能写variable
		self.b1 = nn.Parameter(torch.zeros(4))
		self.w2 = nn.Parameter(torch.randn(4, 1) * 0.01)
		self.b2 = nn.Parameter(torch.zeros(1))
	def forward(self,x):
		x1 = torch.mm(x, self.w1) + self.b1
		x1 = F.tanh(x1) # 使用 PyTorch 自带的 tanh 激活函数
		x2 = torch.mm(x1, self.w2) + self.b2 #后面还可以接一卷积的操作
		return x2

two_network=network()
optimizer = torch.optim.SGD(two_network.parameters(), 1.)
criterion = nn.BCEWithLogitsLoss()
...
for e in range(3000):
    ...
    optimizer.zero_grad()
    loss.backward()
    print("求导:",two_network.w1.grad)
    print("w1是多少:",two_network.w1)
    optimizer.step()
    print("优化后的w1是多少:",two_network.w1)
    if (e + 1) % 1000 == 0:
        print('epoch: {}, loss: {}'.format(e+1, loss.data[0]))
def plot_network(x):
    x = Variable(torch.from_numpy(x).float())
    x1 = torch.mm(x, two_network.w1) + two_network.b1
    x1 = torch.tanh(x1)
    x2 = torch.mm(x1, two_network.w2) + two_network.b2
